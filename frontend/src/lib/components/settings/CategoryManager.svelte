<script>
  ...
  let formData = $state({ name: "", description: "", color: "#6b7280", icon: "" });
  ...
  function startCreate() {
    editing = null;
    formData = { name: "", description: "", color: "#6b7280", icon: "" };
    showForm = true;
  }
  ...
  async function deleteCategory(id) {
    try {
      ...
      console.error("Kategorie loeschen fehlgeschlagen:", e);
    }
  }
      ...
      <div class="row" class:ai={cat.isAi}>
        ...
        <div class="row-actions">
          ...
          {#if confirmDeleteId === cat.id}
            <button class="act danger" onclick={() => deleteCategory(cat.id)} title="Wirklich loeschen">
          ...
          {:else}
            <button class="act danger" onclick={() => (confirmDeleteId = cat.id)} title="Loeschen">
